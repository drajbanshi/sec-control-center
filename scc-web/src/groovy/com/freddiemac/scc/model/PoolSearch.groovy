...
static constraints = {
    ...
    cusipIdentifier  validator: { val, obj ->
            if (val && (val.length() > 9 || val.length() < 9) && !obj.poolNumber) return 'PoolSearch.searchCriteria.cusipIdentifier.size'
    ...
    poolNumber validator: { val, obj  ->
            ...
            if (val && (val.length() > 8 || val.length() < 6))  return 'PoolSearch.searchCriteria.poolNumber.size'
